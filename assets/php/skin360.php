<?php
/*
 * Skin360 (originally MinecraftAvatar)
 * Written by: jamiebicknell
 * Modified by: Gamecrafter
 * Website: http://jamiebicknell.com
 * GitHub: https://github.com/jamiebicknell/Minecraft-Avatar/blob/master/skin.php
 */
$name = isset($_GET["name"]) ? $_GET["name"] : "";
$size = (isset($_GET["size"]) and is_numeric($_GET["size"])) ? max(40, min(800, $_GET["size"])) : 250;
$ac = isset($_GET["ac"]) ? (bool) $_GET["ac"] : true;
function getSkin($name){
    ...
    if($name != ""){
        $output = file_get_contents("http://skins.minecraft.net/MinecraftSkins/".$name.".png");
    }
    return $output;
}
$skin = getSkin($name);
